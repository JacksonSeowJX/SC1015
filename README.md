...
## Project Insights
In this project, data is meticulously prepared and split for various analyses to ensure meaningful and practical insights. For linear regression, the dataset is split to use the first 9 seasons as training data to predict outcomes in the 10th season, the first 10 seasons to predict the 11th, and the first 11 seasons to predict the 12th. This sequential split mirrors real-world scenarios where past performance data is used to forecast future outcomes. In classification tasks, the data from three seasons (2016-2019) is split sequentially, with 80% used for training and 20% for testing, ensuring that the model learns from earlier data to predict later outcomes. For anomaly detection, the focus is on identifying deviations or anomalies during specific seasons, analyzing how singular events or changes impact team performance across different periods. This structured approach allows for a comprehensive assessment of variables influencing sports analytics, particularly in football.
